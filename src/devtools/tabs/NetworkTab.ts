...
const logger = createLogger('NetworkTab')
...
export class NetworkTab {
  ...
  private selectedRequest?: NetworkRequest
  private filter = ''
   ...
   * 失败请求
   */
  failRequest(id: string, error: { error: string; duration?: number }): void {
    const request = this.requests.find(r => r.id === id)
    if (!request) return
   ...
   * 检测请求类型
   */
  private detectRequestType(url: string): string {
    const urlLower = url.toLowerCase()

    if (urlLower.includes('/api/')) return 'xhr'
    ...
    if (/\.(mp4|webm|ogg|mp3|wav)/.test(urlLower)) return 'media'
    if (/\.(woff|woff2|ttf|eot|otf)/.test(urlLower)) return 'font'
    if (urlLower.endsWith('.html')) return 'doc'
   ...
   * 添加请求行
   */
  private addRequestRow(request: NetworkRequest): void {
   ...
   * 更新请求行
   */
  private updateRequestRow(request: NetworkRequest): void {
    const row = this.container?.querySelector(`tr[data-id="${request.id}"]`)
    if (!row) return

    const statusColor = request.error ? '#f44336' :
      request.status && request.status >= 400 ? '#f44336' :
        request.status && request.status >= 300 ? '#ff9800' :
          '#4caf50'
   ...
   * 选择请求显示详情
   */
  private selectRequest(request: NetworkRequest): void {
   ...
   * 获取请求名称
   */
  private getRequestName(url: string): string {
    try {
  ...
  private updateStats(): void {
    ...
    if (countEl) countEl.textContent = `${count} 请求`
    ...
    if (timeEl) timeEl.textContent = `${totalTime.toFixed(0)}ms`
  }
