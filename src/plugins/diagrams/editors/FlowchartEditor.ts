...
import type { DiagramEditor, FlowchartData, FlowchartNode, DiagramOptions } from '../types'
...
export class FlowchartEditor implements DiagramEditor {
  ...
  private addNode(): void {
    ...
    const type = prompt('节点类型 (start/end/process/decision/io):', 'process') as any
  ...
  private deleteNode(index: number): void {
    if (!this.data || !confirm('确定删除？')) return
