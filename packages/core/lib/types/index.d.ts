 ...
 * 定义所有可能的节点类型
 */
export type NodeType = 'doc' | 'paragraph' | 'text' | 'heading' | 'blockquote' | 'codeBlock' | 'bulletList' | 'orderedList' | 'listItem' | 'taskList' | 'taskItem' | 'table' | 'tableRow' | 'tableCell' | 'tableHeader' | 'image' | 'horizontalRule';
/**
 * 标记类型联合类型
 * 定义所有可能的文本标记类型
 */
 ...
 */
export type Command = (state: EditorState, dispatch?: (tr: Transaction) => void, editor?: EditorInstance) => boolean;
/**
 * 事务元数据类型
 */
 ...
 * 定义如何将节点/标记渲染为DOM
 */
export type DOMOutputSpec = string | [string, NodeAttrs?, ...DOMOutputSpec[]] | {
    dom: HTMLElement;
};
/**
 * 属性规范
