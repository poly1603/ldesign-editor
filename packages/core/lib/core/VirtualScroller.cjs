...
class VirtualScroller extends event.EventEmitter {
  constructor(options) {
    super();
    this.items = [];
    ...
    this.scrollHandler = helpers.throttle(() => this.handleScroll(), this.options.scrollThrottle);
  }
  /**
   * 设置要渲染的项目列表
   */
...
//# sourceMappingURL=VirtualScroller.cjs.map
