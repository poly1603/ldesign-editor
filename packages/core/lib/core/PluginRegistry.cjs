...
var PluginCategory = /* @__PURE__ */ ((PluginCategory2) => {
  PluginCategory2["FORMAT"] = "format";
  PluginCategory2["BLOCK"] = "block";
  PluginCategory2["INLINE"] = "inline";
  PluginCategory2["MEDIA"] = "media";
  PluginCategory2["TOOL"] = "tool";
  PluginCategory2["AI"] = "ai";
  PluginCategory2["OTHER"] = "other";
  return PluginCategory2;
})(PluginCategory || {});
class PluginRegistry extends event.EventEmitter {
  constructor() {
    super();
    this.registry = /* @__PURE__ */ new Map();
    this.loadedPlugins = /* @__PURE__ */ new Set();
  }
  /**
   * 设置编辑器实例
   */
  ...
  async loadBatch(names) {
    const sorted = names.map((name) => ({
      name,
      priority: this.registry.get(name)?.config.priority || 0
    })).sort((a, b) => b.priority - a.priority).map((item) => item.name);
   ...
   */
  getEnabled() {
    return Array.from(this.registry.entries()).filter(([_, reg]) => reg.config.enabled).map(([name]) => name);
  }
  /**
   * 按分类获取插件
   */
  getByCategory(category) {
    return Array.from(this.registry.entries()).filter(([_, reg]) => reg.metadata.category === category).map(([name]) => name);
  }
  /**
   * 获取插件统计信息
   */
...
//# sourceMappingURL=PluginRegistry.cjs.map
