...
const logger = logger$1.createLogger("AuditLogger");
class AuditLogger extends event.EventEmitter {
  constructor(config = {}) {
    super();
    this.logs = [];
    this.maxLogs = 1e4;
  ...
  calculatePeakHours(logs) {
    ...
    return Object.entries(hourCounts).sort((a, b) => b[1] - a[1]).slice(0, 3).map(([hour]) => Number(hour));
  }
  /**
   * 分组日志
   */
  ...
  async export(format = "json") {
    ...
    switch (format) {
      ...
      case "pdf":
        return new Blob([this.generateTextReport(logs)], {
          type: "text/plain"
        });
      default:
  ...
  exportCSV(logs) {
    ...
    const rows = logs.map((log) => [log.id, new Date(log.timestamp).toISOString(), log.userId, log.action, log.resource || "", log.result || "", log.metadata?.ip || ""]);
  ...
  generateTextReport(logs) {
    const report = ["\u5BA1\u8BA1\u65E5\u5FD7\u62A5\u544A", "=".repeat(50), `\u751F\u6210\u65F6\u95F4: ${(/* @__PURE__ */ new Date()).toLocaleString()}`, `\u65E5\u5FD7\u6570\u91CF: ${logs.length}`, "", "\u8BE6\u7EC6\u8BB0\u5F55:", "-".repeat(50)];
    logs.forEach((log) => {
...
//# sourceMappingURL=AuditLogger.cjs.map
