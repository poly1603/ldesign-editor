...
const logger = logger$1.createLogger("SSOManager");
class SSOManager extends event.EventEmitter {
  constructor(config) {
    super();
  ...
  startTokenRefresh(token) {
    const refreshTime = token.expiresAt - Date.now() - 5 * 60 * 1e3;
    if (refreshTime > 0) {
...
//# sourceMappingURL=SSOManager.cjs.map
