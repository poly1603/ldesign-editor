...
var lucide = require('./icons/lucide.cjs');
...
class ContextMenuSystem {
  ...
  renderMenuItem(item, container) {
    ...
    if (item.type === "checkbox" || item.type === "radio") {
      ...
      `;
      if (checked) {
        checkIcon.innerHTML = item.type === "radio" ? lucide.getLucideIcon("circle") : lucide.getLucideIcon("check");
      }
    ...
    if (item.submenu && item.submenu.length > 0) {
      ...
      arrow.style.cssText = `
        ...
        justify-content: center;
      `;
      arrow.innerHTML = lucide.getLucideIcon("chevronRight");
...
exports.ContextMenuSystem = ContextMenuSystem;
/*! End of @ldesign/editor-core | Powered by @ldesign/builder */
//# sourceMappingURL=ContextMenuSystem.cjs.map
