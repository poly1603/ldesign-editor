...
function showAIDialog(options) {
    ...
    const createResultContent = (result, original) => {
        const content = DOMUtils.createElement('div', { style: 'max-height: 500px; overflow-y: auto;' });
        // 纠错类型显示对比
      ...
      `;
        ...
        // 添加文本编辑区
        const textareaContainer = DOMUtils.createElement('div', { style: 'margin-top: 15px' });
