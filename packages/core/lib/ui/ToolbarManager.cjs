...
class ToolbarManager extends event.EventEmitter {
  ...
  async createItemElement(item) {
    ...
    if (item.icon) {
      const iconManager = IconManager.getIconManager();
      const icon = iconManager.createIconElement(item.icon, {
        size: 18
      });
   ...
   */
  renderGroups() {
    const sortedGroups = Array.from(this.groups.values()).sort((a, b) => (a.order || 0) - (b.order || 0));
...
//# sourceMappingURL=ToolbarManager.cjs.map
