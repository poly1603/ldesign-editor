...
export class MarkdownPreviewManager {
     ...
     */
    setupScrollSync() {
        if (!this.editor.contentElement || !this.previewElement)
            return;
...
export class MarkdownShortcutsManager {
     ...
     * 检查标题快捷输入（## + 空格）
     */
    checkHeadingShortcut(text, cursorPos, node) {
     ...
     * 检查列表快捷输入（- + 空格）
     */
    checkListShortcut(text, cursorPos, node) {
        const beforeCursor = text.slice(0, cursorPos);
     ...
     * 检查代码块快捷输入（``` + 回车）
     */
    checkCodeBlockShortcut(text, cursorPos, node) {
        if (text.startsWith('```')) {
            // TODO: 创建代码块
        }
    }
 ...
 * 创建Markdown增强插件
 */
export function createMarkdownEnhancedPlugin(config = {}) {
    ...
    return createPlugin({
        ...
        commands: {
            ...
            'markdown:togglePreview': (state, dispatch, editor) => {
                if (previewManager && editor) {
                    const currentMode = editor.markdownPreviewMode || 'none';
                    const nextMode = currentMode === 'none' ? 'side-by-side' : 'none';
                    previewManager.setMode(nextMode);
                    editor.markdownPreviewMode = nextMode;
                    return true;
                }
                return false;
            },
        ...
        init(editor) {
            ...
            if (config.previewMode && config.previewMode !== 'none') {
                ...
                editor.markdownPreview = previewManager;
            }
            // 创建快捷输入管理器
 ...
 * @returns 预览管理器
 */
export function getMarkdownPreviewManager(editor) {
    return editor.markdownPreview || null;
}
