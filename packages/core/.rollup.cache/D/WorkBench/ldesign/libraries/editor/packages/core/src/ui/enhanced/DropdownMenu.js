...
export class DropdownMenu extends BaseComponent {
    ...
    bindItemEvents(element, item) {
        ...
        element.addEventListener('mouseenter', () => {
            if (!item.disabled) {
                this.setFocusedIndex(Number.parseInt(element.dataset.index || '-1'));
    ...
    bindEvents() {
        ...
        document.addEventListener('click', (e) => {
            if (this.isOpen && !this.menuElement.contains(e.target)
                && (!this.config.trigger || !this.config.trigger.contains(e.target)))
                this.close();
        });
        // Close on escape
    ...
    filterItems(searchTerm) {
        if (!searchTerm) {
            ...
            this.filteredItems = this.config.items.filter(item => !item.divider && item.label.toLowerCase().includes(term));
        }
        this.renderItems();
    }
    ...
    expandFocusedSubmenu() {
        if (this.focusedIndex >= 0 && this.focusedIndex < this.filteredItems.length) {
            const item = this.filteredItems[this.focusedIndex];
            if (item.children && item.children.length > 0) {
                const itemElement = this.itemsContainer.querySelectorAll('.dropdown-item')[this.focusedIndex];
                this.showSubmenu(item, itemElement);
            }
        }
    }
    collapseFocusedSubmenu() {
        // Hide all submenus
        this.subMenus.forEach((menu, id) => {
            this.hideSubmenu(id);
        });
    ...
    selectItem(item) {
        if (this.config.multiSelect) {
            ...
            if (itemElement) {
                ...
                const checkbox = itemElement.querySelector('.dropdown-item-checkbox');
                if (checkbox)
                    checkbox.checked = this.selectedItems.has(item.id);
            }
