 ...
 */
function createCodeEditorDialog(onInsert, initialCode = '', initialLanguage = 'javascript') {
    // 创建遮罩层
  ...
  `;
    ...
    // 初始化 CodeMirror
    let editorView = null;
    const initEditor = (language = 'javascript', code = '') => {
        // 清除旧的编辑器
        if (editorView)
            editorView.destroy();
        // 获取语言支持
 ...
 * 代码块插件 - CodeMirror 版本
 */
export const CodeBlockCodeMirrorPlugin = createPlugin({
...
//# sourceMappingURL=codeblock-codemirror.js.map