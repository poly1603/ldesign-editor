...
export async function optimizeBuild(options) {
 ...
 * 获取Terser配置
 */
function getTerserOptions(mode) {
    const baseOptions = {
        compress: {
            ecma: 2020,
 ...
 * 获取esbuild目标
 */
function getEsbuildTarget(target, modern) {
    if (modern)
 ...
 * 应用平台特定优化
 */
function applyPlatformOptimizations(config, target, mode) {
    switch (target) {
        case 'mobile':
            // 移动端优化
 ...
 * 分析构建结果
 */
async function analyzeBuildResult(options) {
 ...
 * 计算构建统计
 */
async function calculateBuildStats(dir) {
    const stats = {
        fileCount: 0,
        totalSize: 0,
        jsSize: 0,
 ...
 * 显示优化建议
 */
function displayOptimizationTips(options) {
    console.log(`\n${chalk.blue('=== 优化建议 ===')}`);
    const tips = [];
...
//# sourceMappingURL=optimize-build.js.map