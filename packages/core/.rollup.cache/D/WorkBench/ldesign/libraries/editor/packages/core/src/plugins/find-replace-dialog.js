...
const showDialog = StateEffect.define();
const updateQuery = StateEffect.define();
const updateReplaceText = StateEffect.define();
const updateOptions = StateEffect.define();
...
const matchDecorations = StateField.define({
    create() {
...
function buildDecorations(state, query, options) {
    if (!query)
        return Decoration.none;
    const decorations = [];
...
function getSearchCursor(state, query, options) {
...
const findReplacePlugin = ViewPlugin.fromClass(class {
    ...
    `;
        ...
        const updateOptionsFromUI = () => {
            const caseSensitive = dialog.querySelector('.cm-case-sensitive')?.checked || false;
            const wholeWord = dialog.querySelector('.cm-whole-word')?.checked || false;
            const regex = dialog.querySelector('.cm-regex')?.checked || false;
            this.view.dispatch({
...
export function openFindReplaceDialog(view) {
    view.dispatch({
...
//# sourceMappingURL=find-replace-dialog.js.map