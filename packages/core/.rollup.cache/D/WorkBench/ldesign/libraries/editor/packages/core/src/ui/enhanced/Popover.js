...
export class Popover extends BaseComponent {
    ...
    calculateBestPlacement(triggerRect, popoverRect) {
        const viewport = {
    ...
    calculatePosition(placement, triggerRect, popoverRect) {
        const offset = this.config.offset;
        let top = 0;
        let left = 0;
        switch (placement) {
    ...
    positionArrow(placement, triggerRect, popoverRect, position) {
        if (!this.arrowElement)
            return;
        // Reset arrow styles
