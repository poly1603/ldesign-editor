...
import ora from 'ora';
export async function analyzePerformance(file, options) {
...
function analyzeData(data, options) {
    const threshold = Number.parseInt(options.threshold || '50');
    // 基础统计
    const stats = {
 ...
 * 生成优化建议
 */
function generateSuggestions(stats, threshold) {
    const suggestions = [];
    // FPS建议
    ...
    // 操作类型建议
    Object.entries(stats.operationsByType).forEach(([type, operations]) => {
 ...
 * 生成报告
 */
async function generateReport(analysis, options) {
 ...
 * 生成HTML报告
 */
async function generateHTMLReport(analysis, outputPath) {
    const html = `
<!DOCTYPE html>
<html>
<head>
 ...
 * 生成文本报告
 */
async function generateTextReport(analysis, outputPath) {
    const lines = [
        '性能分析报告',
        '='.repeat(50),
        '',
        ...
        '',
        '优化建议:',
        ...analysis.suggestions.map(s => `  [${s.level.toUpperCase()}] ${s.category}: ${s.message}\n    建议: ${s.recommendation}`),
        '',
        '慢操作 TOP 10:',
        ...analysis.stats.slowOperations.slice(0, 10).map((op, i) => `  ${i + 1}. ${op.name} - ${op.duration.toFixed(2)}ms`),
    ];
...
//# sourceMappingURL=analyze-performance.js.map