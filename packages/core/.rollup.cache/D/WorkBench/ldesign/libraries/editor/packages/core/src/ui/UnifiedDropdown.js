 ...
 * 显示颜色选择下拉框（简化版）
 */
export function showColorDropdown(button, onSelect, includeCustom = true, title = '选择颜色') {
    // 创建颜色选项
    const colorOptions = PRESET_COLORS.slice(0, 20).map(color => ({
        label: color,
        value: color,
        color,
    }));
 ...
 * 显示表情选择下拉框
 */
export function showEmojiDropdown(button, onSelect) {
    // 创建自定义内容
    const customContent = document.createElement('div');
    // 创建分类标签
...
//# sourceMappingURL=UnifiedDropdown.js.map