...
export class Dropdown extends BaseComponent {
    ...
    setTrigger(trigger) {
        ...
        if (this.dropdownOptions.showOnHover) {
            ...
            this.bindEvent(trigger, 'click', (e) => {
                e.stopPropagation();
                this.toggle();
            });
        }
    ...
    filterItems(query) {
        if (!query) {
            ...
            this.filteredItems = this.items.filter(item => !item.divider && item.label.toLowerCase().includes(lowerQuery));
        }
        this.renderItems();
    }
    beforeShow() {
    ...
    getAlignmentPosition(triggerRect, dropdownRect, alignment) {
        switch (alignment) {
            case 'center':
