...
import { WasmDiff } from './WasmDiff.js';
import { WasmParser } from './WasmParser.js';
...
class WasmAccelerator {
   ...
   */
  getStats() {
    const diffStats = this.diffModule?.getCacheStats() || {
      hits: 0};
    const memoryUsage = this.diffModule?.getMemoryUsage() || {
      used: 0};
    let totalSpeedup = 0;
    let count = 0;
    ...
    const totalTimeSaved = this.stats.diff.nativeTime - this.stats.diff.totalTime + (this.stats.parser.nativeTime - this.stats.parser.totalTime);
    return {
...
export { WasmAccelerator };
/*! End of @ldesign/editor-core | Powered by @ldesign/builder */
//# sourceMappingURL=WasmAccelerator.js.map
