...
export class ConfigTab {
   ...
   * 创建设置部分
   */
  private createSection(section: ConfigSection, items: ConfigItem[]): HTMLElement {
   ...
   * 创建配置项
   */
  private createConfigItem(item: ConfigItem): HTMLElement {
    const container = document.createElement('div')
   ...
   * 创建控件
   */
  private createControl(item: ConfigItem): HTMLElement {
    switch (item.type) {
      case 'boolean':
   ...
   * 创建开关
   */
  private createToggle(item: ConfigItem): HTMLElement {
   ...
   * 创建数字输入
   */
  private createNumberInput(item: ConfigItem): HTMLElement {
    const container = document.createElement('div')
    container.style.cssText = `
      display: flex;
      align-items: center;
   ...
   * 创建文本输入
   */
  private createTextInput(item: ConfigItem): HTMLElement {
    const input = document.createElement('input')
    input.type = 'text'
   ...
   * 创建下拉选择
   */
  private createSelect(item: ConfigItem): HTMLElement {
   ...
   * 创建颜色选择器
   */
  private createColorPicker(item: ConfigItem): HTMLElement {
    const input = document.createElement('input')
    input.type = 'color'
    input.value = item.value
    input.style.cssText = `
   ...
   * 创建JSON编辑器
   */
  private createJsonEditor(item: ConfigItem): HTMLElement {
   ...
   * 应用配置
   */
  private applyConfig(key: string, value: any): void {
    logger.info(`Config updated: ${key} = ${value}`)

    // 更新编辑器配置
    const keys = key.split('.')
    let target: any = this.editor.options || {}
