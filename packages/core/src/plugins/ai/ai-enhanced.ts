...
export class AIEnhancedManager {
  ...
  constructor(editor: EditorInstance, config: AIEnhancedConfig = {}) {
    ...
    if (config.apiKey)
      this.aiService.updateConfig({ providers: { [this.aiService.getCurrentProvider() || 'deepseek']: { apiKey: config.apiKey } as any } })
  }
   ...
   * @returns 排版后的内容
   */
  async smartFormat(): Promise<string> {
...
请直接返回优化后的HTML内容。`
   ...
   * @returns 摘要对象
   */
  async generateSummary(length: 'short' | 'medium' | 'long' = 'medium'): Promise<ContentSummary> {
    const content = this.editor.getHTML()
...
请以JSON格式返回：
{
  ...
  "keyPoints": ["要点1", "要点2", ...]
      ...
      if (response.text || response.content) {
        const result = JSON.parse(response.text || response.content || '{}')
        return {
   ...
   * @returns 关键词列表
   */
  async extractKeywords(count: number = 10): Promise<KeywordResult> {
    const content = this.editor.getHTML()
    const plainText = this.htmlToPlainText(content)
...
其中score范围为-1（极负面）到1（极正面）。`
   ...
   * @returns 处理结果
   */
  async batchProcess(operation: 'proofread' | 'translate' | 'simplify'): Promise<string> {
    const content = this.editor.getHTML()
 ...
 * 创建AI增强插件
 */
export function createAIEnhancedPlugin(config: AIEnhancedConfig = {}) {
  let manager: AIEnhancedManager | null = null
  ...
  return createPlugin({
    ...
    commands: {
      // 智能排版
      'ai:smartFormat': (state, dispatch, editor) => {
        if (manager && editor) {
          manager.smartFormat().catch(err => logger.error('Smart format failed:', err))
          return true
        }
        return false
      },
      ...
      // 生成摘要
      'ai:generateSummary': (state, dispatch, editor) => {
        if (manager && editor) {
          manager.generateSummary().then(summary => {
            logger.info('Summary:', summary)
          }).catch(err => logger.error('Generate summary failed:', err))
          return true
        }
        return false
      },
      ...
      // 提取关键词
      'ai:extractKeywords': (state, dispatch, editor) => {
        if (manager && editor) {
          manager.extractKeywords().then(keywords => {
            logger.info('Keywords:', keywords)
          }).catch(err => logger.error('Extract keywords failed:', err))
          return true
        }
        return false
      },
      ...
      // 情感分析
      'ai:analyzeSentiment': (state, dispatch, editor) => {
        if (manager && editor) {
          manager.analyzeSentiment().then(sentiment => {
            logger.info('Sentiment:', sentiment)
          }).catch(err => logger.error('Analyze sentiment failed:', err))
          return true
        }
        return false
      },
    ...
    init(editor: EditorInstance) {
      ...
      // 暴露到编辑器
      ; (editor as any).aiEnhanced = manager
 ...
 * @returns AI增强管理器
 */
export function getAIEnhancedManager(editor: EditorInstance): AIEnhancedManager | null {
  return (editor as any).aiEnhanced || null
}
