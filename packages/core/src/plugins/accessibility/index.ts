...
export function createAccessibilityPlugin(config: AccessibilityConfig = {}) {
  let manager: AccessibilityManager | null = null
  ...
  return createPlugin({
    ...
    init(editor: EditorInstance) {
      ...
      // 暴露到编辑器
      ; (editor as any).accessibility = manager
 ...
 * @returns 无障碍管理器
 */
export function getAccessibilityManager(editor: EditorInstance): AccessibilityManager | null {
  return (editor as any).accessibility || null
}
